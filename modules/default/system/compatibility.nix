          ...
          substituteInPlace "$out"/bin/appimage-run \
            --replace-fail \
              ...
              'if [ ! -z "$APPIMAGE_USE_NIX_LD" ] && [ "$APPIMAGE_USE_NIX_LD" -eq 1 ]; then
                 ...
                 export XDG_DATA_DIRS="${pkgs.gsettings-desktop-schemas}/share/gsettings-schemas/${pkgs.gsettings-desktop-schemas.name}:${pkgs.gtk3}/share/gsettings-schemas/${pkgs.gtk3.name}"
    ...
    environment = {
      systemPackages = with pkgs; [
        (pkgs.writeShellScriptBin "foxflake-run" ''
          ...
          if [ -e /dev/nvidia0 ] && [ ! "$FOXFLAKE_DISABLE_NVIDIA_FIX" == "1" ]; then
            if [ ! -f /run/opengl-driver/share/vulkan/icd.d/nvidia_icd.json ] || [ ! -f /run/opengl-driver/share/vulkan/icd.d/nvidia_icd.json ]; then
              echo "foxflake-run: NVIDIA GPU detected but not correctly configured, falling back to default."
            else
              ...
              export OCL_ICD_VENDORS="/run/opengl-driver/etc/OpenCL/vendors/nvidia.icd"
          ...
          if [ ! -z "$interp" ] && echo "$interp" | grep -q '^/lib64/ld-linux-x86-64.so'; then
            export JAVA_HOME="${pkgs.jre}"
            export LD_LIBRARY_PATH=""$(dirname "$(realpath "$bin")")"/../libs:"$(dirname "$(realpath "$bin")")"/../lib64:"$(dirname "$(realpath "$bin")")"/../lib:"$NIX_LD_LIBRARY_PATH":"$LD_LIBRARY_PATH""
            export PATH=""$(dirname "$(realpath "$bin")")":"$JAVA_HOME"/bin:"$PATH""
            export QT_XKB_CONFIG_ROOT="${pkgs.xkeyboard_config}/share/X11/xkb"
            export SSL_CERT_FILE="/etc/ssl/certs/ca-bundle.crt"
            export XDG_DATA_DIRS=""$(dirname "$(realpath "$bin")")"/../share:${pkgs.gsettings-desktop-schemas}/share/gsettings-schemas/${pkgs.gsettings-desktop-schemas.name}:${pkgs.gtk3}/share/gsettings-schemas/${pkgs.gtk3.name}:"$XDG_DATA_DIRS""
