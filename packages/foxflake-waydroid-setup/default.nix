...
if ! ${pkgs.curl}/bin/curl -L https://github.com/sebanc/foxflake > /dev/null 2>&1; then ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Internet connection is not available, please make sure you are connected to the internet before using this tool."; exit 1; fi

if [ ''${#} -eq 0 ]; then
	interface=1
elif [ ''${#} -eq 1 ]; then
	if { [ "''${1}" != "VANILLA" ] && [ "''${1}" != "GAPPS" ]; }; then
		${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Error: Waydroid variant is not valid."
		exit 1
	else
		interface=0
	fi
else
	${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Error: Please specify the Waydroid variant you want to install."
	exit 1
fi
...
if [ "''${interface}" -eq 1 ]; then
	...
	if ${pkgs.coreutils}/bin/test "x$(${pkgs.coreutils}/bin/id -u)" != "x0"; then
		pkexec --disable-internal-agent env DISPLAY="''${DISPLAY}" WAYLAND_DISPLAY="''${WAYLAND_DISPLAY}" XAUTHORITY="''${XAUTHORITY}" XDG_RUNTIME_DIR="''${XDG_RUNTIME_DIR}" "''${0}" "''${selected_variant}" || { ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to launch pkexec."; exit 1; }
	fi

else

	if ${pkgs.coreutils}/bin/test "x$(${pkgs.coreutils}/bin/id -u)" != "x0"; then
		pkexec --disable-internal-agent env DISPLAY="''${DISPLAY}" WAYLAND_DISPLAY="''${WAYLAND_DISPLAY}" XAUTHORITY="''${XAUTHORITY}" XDG_RUNTIME_DIR="''${XDG_RUNTIME_DIR}" "''${0}" "''${1}" || { ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to launch pkexec."; exit 1; }
	fi

	echo "Installing Waydroid."

	${pkgs.systemd}/bin/systemctl stop waydroid-container.service || { ${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to stop waydroid-container.service."; exit 1; }

	sleep 5

	${pkgs.coreutils}/bin/rm -rf /var/lib/waydroid /home/*/.local/share/waydroid /home/*/.local/share/applications/*aydroid*

	${pkgs.unstable.waydroid}/bin/waydroid init -s "''${1}" || { ${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to init waydroid."; exit 1; }
	...
	${pkgs.nix}/bin/nix-shell -p bash -p curl -p gnupg -p lzip -p util-linux -p unzip -p xz -p python3 -p python3Packages.inquirerpy -p python3Packages.requests -p python3Packages.tqdm --run "/tmp/waydroid_script/main.py install ''${arm_translation} widevine" || { ${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "waydroid_script failed to install ''${arm_translation} and widevine."; exit 1; }

	${pkgs.unstable.waydroid}/bin/waydroid upgrade -o || { ${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to add waydroid options."; exit 1; }
	${pkgs.systemd}/bin/systemctl restart waydroid-container.service || { ${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --width=800 --title="FoxFlake - Waydroid setup" --error --ok-label="Exit" --text "Failed to restart waydroid-container.service."; exit 1; }
	...
	if [  "''${1}" == "GAPPS" ]; then
		...
		${pkgs.sudo}/bin/sudo --preserve-env=DISPLAY,WAYLAND_DISPLAY,XAUTHORITY,XDG_RUNTIME_DIR -u "$(${pkgs.coreutils}/bin/id -nu "''${PKEXEC_UID}")" ${pkgs.zenity}/bin/zenity --height=600 --width=800 --title="FoxFlake - Waydroid setup" --info --ok-label="Exit" --text "\nWaydroid had been installed with the recommended arm translation layer for your device and widevine support, additional features (Magisk, Tweaks...) can be installed with the waydroid-helper program." --ok-label="Exit" > /dev/null 2>&1
	fi
fi
      '';
    };
